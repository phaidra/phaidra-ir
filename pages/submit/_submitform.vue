<template>
  <div>
    ...
    <v-stepper
      v-else-if="form.sections.length > 0"
      v-model="step"
      non-linear
      class="mt-2"
      alt-labels
    ...
    >
      <v-stepper-header>
        <v-stepper-step
          edit-icon="mdi-check"
          :editable="maxStep >= 1 && step < 8"
          :complete="step > 1"
          step="1"
          ><template v-if="$vuetify.breakpoint.lgAndUp">{{
            $t("Start")
          }}</template></v-stepper-step
        >
        <v-divider></v-divider>
        <v-stepper-step
          edit-icon="mdi-check"
          :editable="maxStep >= 2 && step < 8"
          :complete="touCheckbox"
          step="2"
          ><template v-if="$vuetify.breakpoint.lgAndUp">{{
            $t("Terms of use")
          }}</template></v-stepper-step
        ...
        <v-stepper-step
          edit-icon="mdi-check"
          v-if="submitformparam === 'journal-article'"
          :editable="maxStep >= 4 && step < 8"
          :complete="step > 4"
          step="4"
          ><template v-if="$vuetify.breakpoint.lgAndUp">{{
            $t("Check rights")
        ...
        <v-stepper-step
          edit-icon="mdi-check"
          :editable="maxStep >= 5 && step < 8"
          :complete="step > 5"
          step="5"
          :rules="[() => validationStatus !== 'error']"
          ><template v-if="$vuetify.breakpoint.lgAndUp">{{
            $t("Mandatory fields")
          }}</template>
          <small v-if="validationStatus === 'error'">{{
            $t("Invalid metadata")
        ...
        >
        <v-divider></v-divider>
        <v-stepper-step edit-icon="mdi-check" :complete="maxStep > 8" step="8"
          ><template v-if="$vuetify.breakpoint.lgAndUp">{{
            $t("Notifications")
          }}</template></v-stepper-step
        >
      ...
      <v-stepper-items>
          ...
          <v-container v-if="checkAllowSubmitRes.allowsubmit === 1">
            <v-row no-gutters>
              <h3 class="title font-weight-light primary--text mb-4">
                {{ $t("Submitting a publication") }}
              </h3>
            </v-row>
            <v-row no-gutters>
              <i18n path="SUBMIT_START_1" tag="p">
                <a
                  :href="'/info/policy?lang=' + $i18n.locale"
                  target="_blank"
                  >{{ $t("guidelines") }}</a
                >
              </i18n>
            ...
            <v-row no-gutters justify="center">
              <v-col>
                <v-alert outlined type="error" color="primary">
                  <h3 class="title font-weight-light mb-4 ml-4">
                    {{ $t("Important notice") }}
                  </h3>
                  <ul class="black--text ml-4">
                    <li>
                      <i18n path="SUBMIT_START_2" tag="p">
                        <strong>{{
                          $t("if you hold the necessary rights")
                        }}</strong>
                      </i18n>
                    </li>
                    <li>
                      <i18n path="SUBMIT_START_3" tag="p">
                        <strong>{{
                          $t("a larger number of publications")
                        }}</strong>
                        <a
                          :href="'/info/contact?lang=' + $i18n.locale"
                          target="_blank"
                          >{{ $t("contact us") }}</a
                        >
                      </i18n>
              ...
              <v-col>
                <v-alert outlined type="error">
                  <i18n path="SUBMIT_NOT_ALLOWED" tag="p">
                    ...
                    <a :href="'mailto:' + config.officecontact.email">{{
                      config.officecontact.email
                    }}</a>
          ...
          <v-container>
            ...
            <v-row no-gutters>
              <v-checkbox
                v-model="touCheckbox"
                color="primary"
                :error-messages="touCheckboxErrors"
              >
          ...
          <v-container>
            ...
            >
              <span class="mr-2 black--text"
                >{{ $t("Possible duplicate found") }}:</span
              ><a
                target="_blank"
                :href="
                  'https://' + config.phaidrabaseurl + '/' + doiDuplicate.pid
                "
                >{{ doiDuplicate.title }}</a
              >
            </v-alert>
            ...
            <v-row justify="start" class="mt-2">
              <v-col cols="12" md="5" class="mr-4">
                <v-text-field
                  ...
                  v-on:keyup.enter="journalSearch()"
                ...
                >
                  <template v-slot:item.title="{ item }">
                    <template v-if="Array.isArray(item.title)">{{
                      item.title[0].title | truncate(50)
                    }}</template>
                  </template>
                  <template v-slot:item.publishers="{ item }">
                    <template v-if="Array.isArray(item.publishers)">
                      <template
                        v-if="Array.isArray(item.publishers[0].publisher.name)"
                      >
                        <span
                          v-for="(pub, i) in item.publishers"
                          :key="'pub' + i"
                          ><template v-if="i > 0">, </template
                          >{{ pub.publisher.name[0].name }}</span
                        >
                      </template>
                    </template>
                  </template>
                  ...
                  >
                    <v-btn text color="primary" @click="expand(!isExpanded)">{{
                      $t(isExpanded ? "Unselect journal" : "Select journal")
                    }}</v-btn>
                  </template>
                  <template v-slot:expanded-item="{ headers, item }">
                    <td :colspan="headers.length">
                      <publisher-policy-versions
                        :publication="item"
                      ></publisher-policy-versions>
                    </td>
                  </template>
        ...
        >
          <v-container>
            ...
            >
              <span v-for="(error, i) of validationErrors" :key="'verr' + i">{{
                error
              }}</span>
            </v-alert>
            <v-row>
              <v-col cols="10" offset="1">
                <template v-if="s.id === 6">
                  <submit-ir-description-keywords
                    :label="
                      submitformparam === 'journal-article'
                        ? $t('Article details')
                        : submitformparam === 'book'
                        ? $t('Book details')
                        : $t('Book chapter details')
                    "
                ...
                <template v-for="f in s.fields">
                  <v-row no-gutters :key="f.id">
                    ...
                    <template v-else-if="f.component === 'p-series'">
                      <p-i-series
                        v-bind.sync="f"
                        v-on:input-select-journal="
                          selectJournal(s.fields, f, $event)
                        "
                        v-on:input-title="f.title = $event"
                        v-on:input-title-language="
                          setSelected(f, 'titleLanguage', $event)
                        "
                    ...
                    <template v-else-if="f.component === 'p-bf-publication'">
                      <p-i-bf-publication
                        v-bind.sync="f"
                        v-on:input-suggest-publisher="
                          publisherSuggestInput(f, $event)
                        "
                        v-on:input-publisher-name="f.publisherName = $event"
                        v-on:change-type="f.publisherType = $event"
                        v-on:input-publisher-select="
                          publisherSelectInput(f, $event)
                        "
                    ...
                    <template v-else-if="f.component === 'p-contained-in'">
                      <p-i-contained-in
                        ...
                        v-on:input-series-title-language="
                          setSelected(f, 'seriesTitleLanguage', $event)
                        "
                        ...
                        v-on:input-series-identifier="
                          f.seriesIdentifier = $event
                        "
                        v-on:input-page-start="f.pageStart = $event"
                        v-on:input-page-end="f.pageEnd = $event"
                        ...
                        v-on:add-clear-series="
                          addClearContainedInSeries(f.series, $event)
                        "
                        v-on:remove-series="
                          removeContainedInSeries(f.series, $event)
                        "
                        v-on:add-role="addContainedInRole(f.roles, $event)"
                        v-on:remove-role="
                          removeContainedInRole(f.roles, $event)
                        "
                        v-on:up-role="sortContainedInRoleUp(f.roles, $event)"
                        v-on:down-role="
                          sortContainedInRoleDown(f.roles, $event)
                        "
                        :inputStyle="inputStyle"
                      ></p-i-contained-in>
                    </template>
                    ...
                    <template
                      v-else-if="f.component === 'p-vocab-ext-readonly'"
                    >
                    ...
                    <template v-else-if="f.component === 'isbn'">
                      <p-i-alternate-identifier
                        ...
                        v-on:input-identifier-type="
                          setSelected(f, 'type', $event)
                        "
                        :inputStyle="inputStyle"
                        class="my-2"
                      ></p-i-alternate-identifier>
                    ...
                    <template
                      v-else-if="
                        f.component === 'p-alternate-identifier' &&
                        f.subloopFlag
                      "
                    >
                      <v-row class="my-4">
                    ...
                    <template
                      v-else-if="f.component === 'p-project' && f.subloopFlag"
                    >
                      <v-row class="my-4">
                        <v-col cols="12">
                          <v-card width="100%">
                ...
                <v-row no-gutters v-if="showSubmitWarning">
                  <v-col cols="12">
                    <v-alert type="error" outlined>
                      <span class="mr-2">{{
                        $t("SUBMIT_ATTENTION", {
                          journal: journalSearchSelected[0].title[0].title,
                          version: getLocalizedTermLabel(
                            "versiontypes",
                            selectedVersion
                          ),
                        })
                ...
                <v-divider class="mt-5 mb-7"></v-divider>
                <v-row no-gutters justify="space-between">
                  <v-btn dark color="grey" @click="backForm(s.id)">{{
                    $t("Back")
                  }}</v-btn>
                  <v-btn color="primary" @click="continueForm(s.id)">{{
                    $t("Continue")
                  }}</v-btn>
                </v-row>
              </v-col>
            ...
            <v-row no-gutters>
              ...
              <v-dialog v-model="discardDialog" max-width="500px">
                ...
                <v-card>
                  ...
                  <v-card-text>
                    <p
                      class="mt-6 title font-weight-light grey--text text--darken-3"
                    >
                      {{ $t("Discard submission process?") }}
                    </p>
                  </v-card-text>
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn dark @click="discardDialog = false" color="grey">{{
                      $t("Cancel")
                    }}</v-btn>
                    <v-btn
                      :to="'/'"
                ...
                <v-card>
                  ...
                  <v-card-text>
                    ...
                    >
                      {{
                        $t(
                          "You have been logged out. Please log in to continue submit."
                        )
                      }}
                    </p>
                    <v-col cols="10" offset="1">
                      ...
                      <v-text-field
                        ...
                        single-line
                        :append-icon="
                          loginPassVisible ? 'mdi-eye' : 'mdi-eye-off'
                        "
                  ...
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn dark @click="loginDialog = false" color="grey">{{
                      $t("Cancel")
                    }}</v-btn>
                    <v-btn
                      @click="login()"
        ...
        <v-stepper-content step="8">
          <v-container v-if="submitResponse">
            ...
            <v-row no-gutters>
              <p>
                {{ $t("The persistent URL of your object is") }}
                <a
                  target="_blank"
                  :href="
                    'https://' +
            ...
            <v-row
              no-gutters
              v-if="
                submitResponse.alternatives &&
                submitResponse.alternatives.length > 0
              "
            >
              <p>
                {{ $t("The persistent URLs of alternative versions are") }}:
              </p>
              <p v-for="(a, i) of submitResponse.alternatives" :key="'alt' + i">
                <a
                  target="_blank"
                  :href="'https://' + config.phaidrabaseurl + '/' + a"
                  >{{ "https://" + config.phaidrabaseurl + "/" + a }}</a
                >.
              </p>
            </v-row>
            <template v-if="this.showEmbargoDate">
              <v-row no-gutters>
                <p>
                  {{
                    $t(
                      "Your publication will be available as soon as the embargo period has expired."
                    )
                  }}
                </p>
              </v-row>
                ...
                >
                  <template v-slot:label>
                    <span class="black--text">{{
                      $t(
                        "as soon as my submission has been successfully verified"
                      ) + ","
                    }}</span>
                  </template>
                </v-checkbox>
              ...
              <v-row no-gutters>
                <v-checkbox
                  v-model="embargoNotificationCheckbox"
                  hide-details
                  color="primary"
                >
                  <template v-slot:label>
                    <span class="black--text">{{
                      $t("as soon as the embargo period has expired") + "."
                    }}</span>
                  </template>
                </v-checkbox>
              </v-row>
            ...
            <v-row no-gutters v-else>
              ...
              >
                <template v-slot:label>
                  <span class="black--text">{{
                    $t(
                      "Please notify me by e-mail as soon as my object is fully accessible."
                    )
                  }}</span>
                </template>
              </v-checkbox>
    ...
    <v-dialog v-model="submitBackDialog" width="500">
      <v-card>
        <v-card-title class="headline grey lighten-2">{{
          $t("Submit in progress")
        }}</v-card-title>
        <v-card-text>{{
          $t("Please wait until the submission process has finished.")
        }}</v-card-text>
        <v-divider></v-divider>
        <v-card-actions>
          <v-spacer></v-spacer>
    ...
    <v-dialog v-model="confirmBackDialog" width="500">
      <v-card>
        ...
        <v-card-text>{{
          $t("Please confirm to exit the submit form.")
        }}</v-card-text>
        <v-divider></v-divider>
        <v-card-actions>
          <v-spacer></v-spacer>
      ...
      if (process.browser) {
        ...
        try {
          let response = await axios.get(
            `${this.config.apis.sherparomeo.url}?api-key=${
              this.config.apis.sherparomeo.key
            }&item-type=publication&format=Json&limit=${
              this.journalSearchOptions.itemsPerPage
            }&offset=${
              (this.journalSearchOptions.page - 1) *
              this.journalSearchOptions.itemsPerPage
...
if (crossrefData['issued']['date-parts'][0]) {
    ...
    checkTou: function () {
      if (this.touCheckbox) {
        ...
        this.touCheckboxErrors.push(
          this.$t("You have to accept the terms of use.")
        );
      }
    },
    ...
    submit: async function () {
      ...
      try {
        ...
        if (
          ...
          if (
            response.data.alerts.length === 1 &&
            response.data.alerts[0].msg === "true"
          ) {
    ...
    addEntityClear: function (arr, f) {
      ...
      if (newField) {
        newField.id = new Date().getTime();
        if (
          this.submitformparam === "journal-article" ||
          this.submitformparam === "book-part"
        ) {
    ...
    selectInput: function (fields, f, event) {
      if (event) {
        ...
        if (f.predicate === "dcterms:accessRights") {
          this.showEmbargoDate =
            f.value === "https://pid.phaidra.org/vocabulary/AVFC-ZZSZ";
        }
    ...
    checkSeriesPosition: function () {
      ...
      if (
        objectType === "https://pid.phaidra.org/vocabulary/VKA6-9XTY" &&
        versionType !== "https://pid.phaidra.org/vocabulary/TV31-080M"
      ) {
    ...
    moveSeriesTo: function (moveTo) {
      ...
      let sidx = this.getSeriesIdx(
        moveTo === "mandatory" ? optionalSection : mandatorySection
      );
      let otidx = this.getObjectTypeIdx();
      let aiidx = this.getAltIdsIdx();
      if (sidx > -1) {
        if (moveTo === "mandatory") {
          optionalSection.fields[sidx].titleBackgroundColor =
            this.config.mandatorybgcolor;
          mandatorySection.fields.splice(
            otidx + 1,
            0,
            optionalSection.fields[sidx]
          );
          optionalSection.fields.splice(sidx, 1);
        }
        if (moveTo === "optional") {
          mandatorySection.fields[sidx].titleBackgroundColor = undefined;
          optionalSection.fields.splice(
            aiidx + 1,
            0,
            mandatorySection.fields[sidx]
          );
    ...
    validateMandatory: function () {
        ...
        if (s.obligation === "mandatory") {
          for (let f of s.fields) {
                ...
                if (f.type === "schema:Organization") {
                  ...
                  if (f.organizationType === "other") {
                    if (f.organizationText.length < 1) {
                      f.organizationTextErrorMessages.push(
                        this.$t("Missing organization")
                      );
                      this.validationStatus = "error";
                    }
            ...
            if (f.component === "p-date-edtf") {
              if (
                (this.showEmbargoDate || f.type !== "dcterms:available") &&
                !f.hidden
              ) {
                f.typeErrorMessages = [];
            ...
            if (f.component === "p-contained-in") {
              ...
              if (f.publisherType === "other") {
                if (f.publisherName.length < 1) {
                  f.publisherNameErrorMessages.push(
                    this.$t("Missing publisher")
                  );
                  this.validationStatus = "error";
                }
      ...
      if (!hasLocalAffiliation) {
        ...
        this.validationErrors.push(
          this.$t("ERROR_AFFILIATION", {
            institution: this.$t(this.config.institution),
          })
        );
      }
    },
    confirm: async function () {
      ...
      try {
        ...
        if (
          this.submitResponse.alternatives &&
          this.submitResponse.alternatives.length > 0
        ) {
          httpFormData.append(
            "alternatives[]",
            this.submitResponse.alternatives
        ...
        if (this.embargoNotificationCheckbox) {
          httpFormData.append(
            "embargonotification",
            this.embargoNotificationCheckbox
          );
        }

        let response = await axios.post(this.$store.state.config.api + "/ir/notifications", httpFormData, {
           headers: {
            "Content-Type": "multipart/form-data",
            "X-XSRF-TOKEN": this.$store.state.user.token,
          }
    ...
    if (this.step === 8 && to.name === "submit") {
      ...
      if (this.step === 7 && to.name === "submit" && this.loading) {
        ...
        if (
          this.step > 1 &&
          to.name === "submit" &&
          !this.$store.state.skipsubmitrouteleavehook
        ) {
